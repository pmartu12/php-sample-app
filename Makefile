...
# Setup ————————————————————————————————————————————————————————————————————————————————————————————————————————————————
docker := $(DOCKER_CLI) compose
docker_exec := $(DOCKER_CLI) compose exec
php_container := php-fpm
phpstan_container := phpstan
phpcs_container := phpcs
node_container := node
compose	:= $(docker) --file docker-compose.yaml --file docker-compose.override.yaml
...
prepare: hooks ## Prepare the dev environment
	@test -s docker-compose.override.yaml || cp docker-compose.override.dist.yaml docker-compose.override.yaml
.PHONY: prepare
...
test-filter: export APP_ENV = test
test-filter: export APP_DEBUG = 1
test-filter:
	$(docker_exec) -e APP_ENV -e APP_DEBUG $(php_container) vendor/bin/phpunit --configuration phpunit.xml.dist --testdox --stop-on-failure --filter $(call args);
.PHONY: test-filter
